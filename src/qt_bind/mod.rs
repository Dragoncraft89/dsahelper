...
extern "C" {
    fn create(obj: *mut c_void, signal: *const c_char, arg: *mut c_void, callback: *const c_void, argument: *const c_void) -> *mut c_void;
...
impl BindManager {
    ...
    pub fn connect(&mut self, obj: *mut Object, s: &'static [u8], arg: *mut c_void , callback: *mut c_void, argument: *mut c_void) {
        if let Some(var) = self.connections.insert((obj, s), unsafe {create(transmute(obj), s.as_ptr() as *const c_char, transmute(arg), callback, argument)}) {
            unsafe { destroy(var) }
        };
    }
...
macro_rules! connect {
    ($sender:expr, $signal:expr, $data:expr, $call_type:ident, $callback:path) => {
        {
            extern "C" fn temp_call(target: *mut std::os::raw::c_void, _arg: *mut std::os::raw::c_void) {
                unsafe {
            ...
            unsafe {
                qt_bind::MANAGER.lock().unwrap().connect($sender, $signal, std::mem::transmute(data), temp_call as *mut std::os::raw::c_void, 0 as *mut std::os::raw::c_void)
            }
        }
    };
    ($sender:expr, $signal:expr, $data:expr, $call_type:ident, $callback:path, $argument:expr, $argtype:ty) => {
        {
            ...
            let data: &mut $call_type = $data;
            unsafe {
                let ptr = std::boxed::Box::into_raw(std::boxed::Box::new(arg)) as *mut std::os::raw::c_void;
                qt_bind::MANAGER.lock().unwrap().connect($sender, $signal, std::mem::transmute(data), temp_call as *mut std::os::raw::c_void, ptr)
            }
        }
    }
}
